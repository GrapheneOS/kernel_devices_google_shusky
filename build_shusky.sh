...
tools/bazel run ${parameters} --config=stamp --config=shusky --config=fast //private/devices/google/shusky:zuma_shusky_dist "$@"
sign_file=$(mktemp)
trap '{ rm -f -- "$sign_file"; }' EXIT
gcc aosp/scripts/sign-file.c -lssl -lcrypto -o ${sign_file}
find out/shusky/dist -type f -name "*.ko" \
